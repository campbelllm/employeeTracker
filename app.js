...
const allEmployees = () => {
  const allEmployeeQuery =
    "SELECT first_name,last_name,title,salary,name,manager FROM employee, employee_role, department WHERE employee.role_id = employee_role.id AND employee_role.department_id = department.id;";
...
const employeeList = () => {
  ...
  connection.query(employeeListQuery, (err, res) => {
    if (err) throw err;
    res.forEach((name) =>
      nameArray.push(name.first_name + " " + name.last_name)
    );
  });
  return nameArray;
};
...
const addEmployee = () => {
  inquirer
    ...
    .then((answer) => {
      ...
      const getRoleId = `SELECT id FROM employee_role WHERE title = ${JSON.stringify(
        answer.employeeRole
      )}`;
...
const addRole = () => {
  inquirer
    ...
    .then((answer) => {
      const getDepartmentId = `SELECT id FROM department WHERE name = ${JSON.stringify(
        answer.department
      )}`;
      // then make a connection to the db and set the id to a variable
...
const deleteEmployee = () => {
  inquirer
    ...
    .then((answer) => {
      const deleteQuery = `DELETE FROM employee WHERE CONCAT(first_name, ' ', last_name) = ${JSON.stringify(
        answer.deleteName
      )}`;
